...
let
  ...
  terminalPkg = config.xeta.terminal.emulator.package;
  terminal = "${pkgs.kitty}/bin/kitty";
  # screenshot = config.xeta.desktop.screenshots.default;
  screenshot = pkgs.grim;
  ...
  config = mkIf cfg.enable {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        misc =
          let
            terminalStr = if (terminalPkg == pkgs.kitty) then "kitty" else "alacritty";
          in
          {
            vrr = 2;
