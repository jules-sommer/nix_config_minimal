...
$env.config = {
    ...
    recursion_limit: 40 # the maximum number of times nushell allows recursion before stopping it
    ...
    hooks: {
        ...
        env_change: {
            PWD: [
                {
                    # This hook manages adding directories to zoxide, the conditional is used to exlude directories that pollute the db.
                    condition: {|before, after| not ($after | path split | any {|x| $x == "nix" or $x == "store" })}
